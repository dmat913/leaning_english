          ...
          <WordDetailCard className="p-3 gap-2 flex flex-col">
            ...
            <p className="text-black-1 flex flex-col">
              <span className="flex items-center gap-2">
                <IoPlayCircleSharp
                  onClick={() =>
                    handlePlayAudio(level600Data[selectedIndex].word)
                  }
                  size={24}
                />
                {`${level600Data[selectedIndex].id} ${level600Data[selectedIndex].word}`}
              </span>
              <span>{level600Data[selectedIndex].wordMeaning}</span>
            </p>
            <p className="flex flex-col">
              <span className="flex gap-2">
                <IoPlayCircleSharp
                  size={24}
                  onClick={() =>
                    handlePlayAudio(level600Data[selectedIndex].sentence)
                  }
                />
                {level600Data[selectedIndex].sentence}
              </span>
              <span>{level600Data[selectedIndex].sentenceMeaning}</span>
            </p>
            <p className="flex gap-2">
              {level600Data[selectedIndex].portOfSpeech.map((item, index) => (
                <div
                  key={index}
